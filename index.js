...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `salve @${num.split('@')[0]}\ bv ao grupo *${mdata.subject}* se apresente e mama o arabe`
				...
				teks = `mais um saiu @${num.split('@')[0]} se voltar vai ter que mamar dnv`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: 'Espera ai minha net ta ruim',
				success: '️ta ai chatão',
				error: {
				...
				only: {
					...
					ownerG: 'COMANDO DO ARABE KK. ',
					ownerB: 'COMANDO DO ARABE KK. ',
					admin: 'ALA ACHANDO Q É ADM KKKK ',
			...
			switch(command) {
                ...
                case 'hidetag':
					...
					if (!isOwner) return reply('sai dae mano kkk')
...
case 'lofi':
                ...
                case 'reportarbug':
                    ...
                    client.sendMessage('5515998130097@s.whatsapp.net', options, text, {quoted: mek})
				...
				case 'audio':
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							if (err) return reply('falhei desculpe-me tente novamente dps')
       ...
       client.sendMessage(from, 'Este é o número do meu criador ^-^ ',MessageType.text, { quoted: mek} )
           ...
           case 'rebaixar':
			...
			case 'remover':
				...
				case 'adms':
					if (!isGroup) return reply(mess.only.group)
					teks = `aqui esta os gostosos *${groupMetadata.subject}*\nadms : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'converter':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('burrao kkkk tenta dnv ')

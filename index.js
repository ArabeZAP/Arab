...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `salve @${num.split('@')[0]}\ bem vindo/a ao *${mdata.subject}* se apresente e da uma mamadinha no arabe`
				...
				teks = `Flw @${num.split('@')[0]} se entrar dnv vai ter mamar dnv`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: 'Aguarde, minha net é ruim',
				success: '️Pronto gostosa',
				error: {
					stick: 'Deu certo não',
					Iv: 'Desculpe, o link está inválido🥺'
				},
				only: {
					group: 'Esse comando é para grupo ',
					ownerG: 'Esse comando é para o Arabe ',
					ownerB: 'Esse comando é para o Arabe',
					admin: 'Ala o cara acha q é adm kkkk ',
					Badmin: 'O BOT PRECISA SER ADEMAR '
				}
			}
...
case 'lofi':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️Lofi kkkkkkkk sefude lek'})
					break
                 case 'logoph':
					var gh = body.slice(9)
					var gbl1 = gh.split("|")[0];
